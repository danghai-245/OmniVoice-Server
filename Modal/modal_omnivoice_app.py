...
image = (
    ...
    .pip_install(
        ...
        "huggingface_hub>=0.32.0", "hf_xet", "pydub"
    )
...
class OmniVoiceModel:
    ...
    def generate(self, data: dict = None):
        ...
        try:
            ...
            # Voice cloning bắt buộc phải có file mẫu. Không cho phép rơi vào
            # Auto mode vì khi đó OmniVoice có thể tự chọn voice khác cho từng request.
            if not temp_ref_path or not os.path.exists(temp_ref_path):
                return JSONResponse(
                    status_code=400,
                    content={"error": "Không tải được file voice mẫu; request bị từ chối để giữ đúng giọng đã chọn"},
                    headers=headers,
                )
            ...
            if language and str(language).strip().lower() in ["auto", "none", ""]:
                language = None
            ...
            try:
                ...
                try:
                    temp_val = float(data.get("class_temperature") or data.get("temperature") or data.get("temp") or 0.1)
                except (TypeError, ValueError):
                    temp_val = 0.1
                ...
                gen_kwargs = {
                    ...
                    "class_temperature": temp_val,
                    "denoise": bool(data.get("denoise", True))
                }
                if language:
                    gen_kwargs["language"] = str(language).strip()
                ...
                print(f"[*] [OmniVoice] Đang sinh âm thanh nhái giọng (lang={language}, kwargs={list(gen_kwargs.keys())})...")
