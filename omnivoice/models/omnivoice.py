...
class OmniVoice(PreTrainedModel):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if short_idx:
            ...
            short_results = self._generate_iterative(short_task, gen_config, progress_callback=progress_callback)
            for idx, res in zip(short_idx, short_results):
                results[idx] = res

        if long_idx:
            long_task = full_task.slice_task(long_idx)
            long_results = self._generate_chunked(long_task, gen_config, progress_callback=progress_callback)
    ...
    def _generate_chunked(
        self, task: GenerationTask, gen_config: OmniVoiceGenerationConfig, progress_callback: Optional[Callable[[int, int], None]] = None
        ...
        def _run_batch(indices, texts, ref_audios, ref_texts):
            ...
            gen_tokens = self._generate_iterative(sub_task, gen_config, progress_callback=progress_callback)
    ...
    def _estimate_target_tokens(self, text, ref_text, num_ref_audio_tokens, speed=1.0):
        ...
        # Đối với các thẻ biểu cảm [sigh], [laughter],...
        # Mạng ước lượng thời lượng có thể không tính thời gian phát biểu cảm,
        # vì vậy ta bù thêm khoảng 25 tokens (~0.5s) cho mỗi thẻ biểu cảm để tránh mất chữ cuối câu.
        num_tags = len(_NONVERBAL_PATTERN.findall(text))
        if num_tags > 0:
            est += num_tags * 25.0
    ...
    def _generate_iterative(
        ...
        for step in range(gen_config.num_step):
            if progress_callback is not None:
                try:
                    progress_callback(step + 1, gen_config.num_step)
                except Exception:
                    pass
            batch_logits = self(
